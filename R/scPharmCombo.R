...
#' @param drug_info file for all drugs, automatically loaded in scPharm.
#'
...
scPharmCombo <- function(object,
  ...
  for (i in seq(1, nrow(score))) {
    col.name <- c(
      paste("scPharm_label", score[i, "DRUG_ID"], score[i, "DRUG_NAME"], sep = "_"),
    ...
    if (max(out.mix$mu) > 0 && nrow(sub.meta1) != 0) {
      ...
      combo.overlap <- data.frame(
        DRUG_FIRST = score[i, "DRUG_NAME"],
        DRUG_ID = sapply(strsplit(names(combo.overlap), split = "_"), function(x) {
          x[3]
        }),
    ...
    if (!grepl("Other|Unclassified", target.path)) {
      ...
      combo.enhance <- data.frame(
        DRUG_FIRST = score[i, "DRUG_NAME"],
        DRUG_ID = sapply(strsplit(names(combo.enhance), split = "_"), function(x) {
          x[3]
        }),
